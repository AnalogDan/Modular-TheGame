VideoLastState = Class{__includes = BaseState}

function VideoLastState:init()
    ...
    self.sequence = {
        {
            ...
            blipSet = 'teacherBlip',
            sound = 'vid11',
            loopSound = 'forestMusic',
            dialogue = {
                "Escuché un alboroto, ¿todo bien?",
                "...",
                "Okay, recuerda mañana llegar temprano al exámen.",
            }
        },
        {   intro = gVideos['vid22'], 
            killSound = true,
    ...
    Sound.playSFX('vid11')
    Sound.playTrack("children", "ambience", { fadeIn = 3, fadeOut = 5, loop = true, volume = 0.2 })
...
function VideoLastState:skipVideo()
    --self.state = "done"
...
function VideoLastState:update(dt)
    updateBlipTimers(dt)
    ...
    if self.state == "intro" then
        if not current.intro:isPlaying() then -- When finishing the sequence
            if current.isEnd and not self.endingTriggered then
                self.endingTriggered = true
                self.state = "done"
                SystemTransition.start('cover', function()
                    gStateMachine:change('menu')
                    SystemTransition.drawAt1080p = false
...
function VideoLastState:render()
    local video
    if self.state == "intro" then
