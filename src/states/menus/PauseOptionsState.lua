PauseOptionsState = Class{__includes = BaseState}

function PauseOptionsState:init()
...
function PauseOptionsState:applyFullscreen()
    push:setupScreen(
        VIRTUAL_WIDTH,
        VIRTUAL_HEIGHT,
        WINDOW_WIDTH,
...
function PauseOptionsState:adjustSlider(direction)
    local step = 0.02
...
function PauseOptionsState:update(dt)
    ...
    if love.keyboard.wasPressed('enter') or love.keyboard.wasPressed('return') or love.keyboard.wasPressed('space') then
        if self.selected == 1 then
            gStateMachine:pop()
            gStateMachine:push('pause', { isQuickPause = true })
            return
...
function PauseOptionsState:render()
    -- dark overlay
    love.graphics.setColor(0, 0, 0, 0.5)
    love.graphics.rectangle("fill", 0, 0, VIRTUAL_WIDTH, VIRTUAL_HEIGHT)
    love.graphics.setColor(1, 1, 1, 1)

    love.graphics.draw(gTextures['pauseOpsBg'], 2, 0)
    ...
    drawFadingText(self.sfxVolume, 201, 34, self.sfxDisplayTimer)
    drawFadingText(self.musicVolume, 201, 64, self.musicDisplayTimer)
    love.graphics.setColor(1, 1, 1, 1)
